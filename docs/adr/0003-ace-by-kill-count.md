...
## 2026-08-18 — дневной пост возвращён, недельные счётчики остаются

Owner call: дневной пост оказался нужен — недельных счётчиков мало, хочется
видеть сами моменты в день, когда они случились. Возвращаем ОБА формата:

- `src/server/digest-daily/` восстановлен из истории (состояние на удаление,
  #329) — тот же пост «🍿 Эйсы и ножи за предыдущие 24 часа» в 23:00 Киева,
  с хронологическим списком (время, игрок, агент, 🏆/💀, раунд, ссылка на
  матч) и дедупом по киевской дате в `daily_digest_runs`.
- Единственное отличие от старого поста: строка легенды и эмодзи «🔪🪿
  распотрошил гуся» НЕ вернулись — сплит по AFK-жертве отменён насовсем
  (секция 2026-08-04 выше), детектор больше не пишет `victims_afk`.
- Недельные «Эйсы недели» / «Ножи недели» не тронуты. Конфликта нет по
  построению: оба читают одни и те же строки `detected_events` по типу и окну
  дат, никто из них не меняет статус событий.
- `EVENT_CATEGORY` не менялся: `ace` / `knife_kill` остаются `'weekly'`
  (вставка `status='digest-only'`), дневной билдер читает их по типу.

## 2026-09-07 — дневной пост: счётчики по игрокам («🍿 Daily Ace/Knife»)

Owner call по скриншоту: построчный формат дневного поста (ранг + ник#тег +
агент + иконка карты + ссылка + номера раундов) на телефоне читается как грязь —
`<h2>` и почти каждая строка переносятся. Владелец задал замену напрямую, этап
прототипов пропущен (#365, PR #367):
...
- Rich HTML: `<h2>` + `<h3>` секции + `<ul><li>`. Одна строка = один игрок за
  сутки, `×N` при N > 1. Без иконки ранга, агента, карты, ссылок и номеров
  раундов — всё это владелец убрал.
- Счёт — тот же, что у недельных лидербордов (`readOccurrences` из
  `digest/ace-knife.ts`, теперь экспортируется): эйс за раунд, нож за
  килл. Секция 2026-08-18 в части «день — поимённый список моментов» устарела:
  день — тоже счётчики, только за сутки, и дневные цифры складываются в
  недельные.
- Сортировка по счёту убыв., при равенстве — по времени первого события.
- Текстовый фолбэк `renderDailyDigestText` не менялся — расхождение с rich
  ведёт #347.
- `/test_daily_digest [N]` — owner-превью дневного поста в личке (до этого
  дневного превью не было).

См. `src/server/digest-daily/rich-render.ts`.


## 2026-09-11 — дневной пост: моменты по игрокам, эмоджи-ссылка

Владелец задал готовый формат (#369): заголовок «🍿 Эйсы и ножи за предыдущие
24 часа», обычные (не жирные) «🎯 Эйсы» / «🔪 Ножи», строки
`- ранг Ник#Тег агент/карта`. Ник остаётся жирным по общей конвенции.

- Одна строка на эйс-раунд / убийство ножом, в том числе два ножа в одном
  раунде дают две строки. Счёт общий с недельными досками (`readOccurrences`).
- В каждом разделе строки одного игрока идут подряд; группы сортируются по
  суммарному количеству убыв., при равенстве — первое обнаруженное событие.
  Идентичность игрока — PUUID. Каждый момент сохраняет ранг, агента, карту и
  ссылку своего матча.
- Агент и карта через `/` находятся внутри одной ссылки на матч tracker.gg;
  названия карт, время, номера раундов, исходы, легенда и `×N` не печатаются.
  Неизвестные агент/карта заменяются на 🦸/⛰️; неизвестный ранг пропускается
  по общей конвенции `renderPlayerName`.
- Доставка — обычный `sendMessage` с HTML. Зафиксированное ограничение rich
  (`player-render.ts`): `<a>` с `<tg-emoji>` внутри теряет кликабельность.
  Поэтому daily больше не использует rich; старый хронологический fallback
  удалён. Плановая отправка и `/test_daily_digest` используют один текст,
  устраняя расхождение #347.

Длинные списки делятся на сообщения по границам строк с учётом лимита
Telegram 4096 символов после HTML-разбора; ссылки и эмоджи не разрезаются.
Окно, расписание и дедуп не меняются. Недельные доски остаются счётчиками.


### 2026-09-11 — уточнение по скриншоту владельца

Главный заголовок должен остаться `h2`; «🎯 Эйсы» и «🔪 Ножи» — обычный
текст с пустой строкой до списка. Возвращена rich-отправка с `<h2>` и отдельными
абзацами секций и списков; данные строк едины с классическим резервом.
Ранг исправлен на `match_records.rank_after`: сканер пишет туда `player.tier.name`,
а `rank_before` всегда оставляет NULL. Регрессионный тест повторяет эту форму данных.
Эмоджи агента/карты сохраняются внутри ссылки, как потребовал владелец; ранее
зафиксированное ограничение их кликабельности в rich остаётся клиентским риском.


### 2026-09-11 — явные переносы в rich

Скриншот владельца подтвердил: отдельные `<p>` не дают пустую строку
в Telegram. Теперь переносы тела переводятся напрямую в `<br>`, поэтому
после заголовков секций и между секциями стоит `<br><br>`. Главный `h2`
сохраняется. Тест проверяет именно явные переносы отправляемого rich HTML.
