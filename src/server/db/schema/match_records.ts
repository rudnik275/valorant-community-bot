...
export const matchRecords = sqliteTable(
  'match_records',
  {
    ...
    // Count of rounds in this match where the player died and was the FIRST
    // teammate to die among ≥2 dying teammates (min `time_in_round_in_ms`
    // among same-team victims with at least one other team-victim).
    // Null when the match has no qualifying round.
    died_first_rounds: integer('died_first_rounds'),
