...
describe('giantSlayerDetector', () => {
  it('detects giant slayer when enemy is 2 macro tiers higher (Silver vs Platinum)', async () => {
    // Silver=3, Platinum=5, delta=2 → qualifies (≥2 macro tiers)
    const events = await giantSlayerDetector.detect(BASE_RECORD, []);
    expect(events).toHaveLength(1);
    expect(events[0]!.type).toBe('giant_slayer');
    expect(events[0]!.payload.delta).toBeGreaterThanOrEqual(2);
  });

  it('detects giant slayer at exact threshold 2 (Silver vs Platinum boundary)', async () => {
    ...
    const events = await giantSlayerDetector.detect(record, []);
    expect(events).toHaveLength(1);
  });

  it('detects giant slayer: Gold I vs Diamond III (delta=2 macro, ignoring subtier)', async () => {
  ...
  it('does NOT emit when delta is 1 (Gold III vs Platinum I — 1 macro tier)', async () => {
    ...
    expect(await giantSlayerDetector.detect(record, [])).toHaveLength(0);
  });

  it('does NOT emit when delta is less than 2 (1 tier difference — Silver vs Gold)', async () => {
  ...
  it('does NOT emit when result is not win', async () => {
    const record: MatchRecord = { ...BASE_RECORD, result: 'loss' };
    expect(await giantSlayerDetector.detect(record, [])).toHaveLength(0);
  });

  it('does NOT emit when rank is unknown/null', async () => {
    const record: MatchRecord = { ...BASE_RECORD, rank_after: null };
    expect(await giantSlayerDetector.detect(record, [])).toHaveLength(0);
  });

  it('does NOT emit when enemy_avg_rank is null', async () => {
